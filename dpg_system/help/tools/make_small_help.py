...
demo = [
    ...
    {'key': 'c0', 'comment': True, 'text': 'send 0, 1, 2 to ask about each screen\nthe OFFSETS are the important part:\nscreens sit side by side in one big\ncoordinate space, so a second screen\nmay start at x 2560, not x 0\nLinux reads xrandr; macOS needs Quartz\nfrom pyobjc. Without it you get nothing\nand a printed reason - not no screens',
     'pos': (30, 425)},
]
...
demo = [
    ...
    {'key': 'c0', 'comment': True, 'text': 'no inlets and no outlets - the node\nloads a dataset and holds it, and\nnothing in a patch can ask it anything\ngroundwork for a training setup driven\nfrom code, not a way to get data into\na patch',
     'pos': (30, 155)},
