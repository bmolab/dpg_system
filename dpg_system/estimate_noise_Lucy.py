...
Usage:
    python estimate_noise_Lucy.py file1.npz [file2.npz ...]
    python estimate_noise_Lucy.py --dir /path/to/npz_files
    python estimate_noise_Lucy.py file.npz --json results.json
"""
...
def find_clusters(frames, gap=5):
    ...
    for f in frames[1:]:
        if f - end <= gap:
            ...
            clusters.append((start, end)); start = end = f
...
def analyze_file(filepath, verbose=True):
    ...
    elif "mocap_frame_rate" in d:  # GRAB folder naming
        fps = float(d["mocap_frame_rate"])
    else:
        raise KeyError(
            f"{filepath}: expected 'mocap_framerate' or 'mocap_frame_rate'"
        )
...
def main():
    ...
    if args.json:
        out = []
        reports = sorted(reports, key=lambda r: -r.noise_score) # sort the files in json from most problematic
        for r in reports:
            d = asdict(r)
