...
def dump_amass2pytroch(datasets, amass_dir, out_posepath, logger = None, rnd_seed = 100, keep_rate = 0.01):
    ...
    print('Creating pytorch dataset at %s' % out_posepath)
    ...
    for ds_name in datasets:
        ...
        print('randomly selecting data points from %s.' % (ds_name))
        for npz_fname in tqdm(npz_fnames):
            try:
                cdata = np.load(npz_fname)
            except:
                print('Could not read %s! skipping..'%npz_fname)
                continue
            N = len(cdata['poses'])
            ...
            data_pose.extend(cdata['poses'][cdata_ids].astype(float))
            data_dmpl.extend(cdata['dmpls'][cdata_ids].astype(float))
            data_trans.extend(cdata['trans'][cdata_ids].astype(float))
            data_betas.extend(np.repeat(cdata['betas'][np.newaxis].astype(float), repeats=len(cdata_ids), axis=0))
...
def prepare_amass(amass_splits, amass_dir, work_dir, logger=None):
    ...
    print('Stage II: augment the data and save into h5 files to be used in a cross framework scenario.')
    ...
    for split_name in amass_splits.keys():
        ...
        print('%s has %d data points!' % (split_name, len(ds)))
        dataloader = DataLoader(ds, batch_size=batch_size, shuffle=False, num_workers=8, drop_last=False)
    ...
    print('Dumped final pytorch dataset at %s' % stageIII_outdir)
...
if __name__ == '__main__':
    ...
    work_dir = makepath('../AMASS/DataSet/')

    # logger = log2file(os.path.join(work_dir, '%s.log' % (expr_code)))
    print('[%s] AMASS Data Preparation Began.')
    print(msg)

    amass_splits = {
        'validate': ['HumanEva', 'MPI_HDM05', 'SFU', 'MPImosh'],
        'test': ['Transitions'], #, 'SSM_synced'],
        'train': ['CMU', 'MPILimits', 'TotalCapture', 'ACCAD', 'DanceDB', 'DFAUST', 'HUMAN4D', 'PosePrior']#ACCAD
    }
    amass_splits['train'] = list(set(amass_splits['train']).difference(set(amass_splits['test'] + amass_splits['validate'])))

    prepare_amass(amass_splits, amass_dir, work_dir)