...
# We want a node that allows navigation to a place in the oscquery landscape and instantiate
# the interface nodes implied in the json structure
# a key question is how to instantiate a multi-ui interface, in particular, how to place them in the patch
# the root position is at the origin of this navigation node, (like placeholder node)
# do we add relative positions to the elements in subtrees of the oscquery structure?
# relative x, y positions (cumulative across the json structure
    ...
    if isinstance(osc_spec, dict):
        ...
        for type in types:
            ...
            if type == 'f': # float
                ...
                if 'REL_POS' in osc_spec:
                    rel_pos = osc_spec['REL_POS']
